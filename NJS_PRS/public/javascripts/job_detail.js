...
$('.btn_apply').on('click', function(){
    ...
    if(!cookie) inst.open();
        ...
        $.colorbox({inline:true, href:$("#sendNoResume"),title:"投个简历"});
	} else{
        $.ajax({
            url: '/job/apply',
            type:'POST',
            ...
            if(!res.error){
                ...
                // $("#deliverResumesSuccess p.count").html('投递成功');
                // $.colorbox({inline:true, href:$("#deliverResumesSuccess"), title:"投个简历"});
            }else{
                if(res.code == 1){
                    errorTipsSet("投递失败: 简历不完善", "投个简历");
                }else{
                    errorTips("投递失败：" + res.error, "投个简历");
                }
            }
        });
	}
...
$('.jd_collection').on('click', function(){
    var inst = $('[data-remodal-id=login]').remodal();
    var cookie = $('#applicantname').val(),
    	_this = $(this);
    if(!cookie) inst.open();
    else if(_this.hasClass('collected')){
        _this.children('.jd_collection_success').stop(true,true).fadeIn(200).delay(3000).fadeOut(200);
    }else{
        $.ajax({
            url: '/job/collect',
	...
	$.colorbox({
		html:'<div class="popup" style="width:460px;">'+
				'<table width="100%">'+
					...
					'</tr>'+
					'<tr>'+
						'<td align="center"><a href="/resume" target="_blank" class="btn_s">马上去完善</a></td>'+
					'</tr>'+
				'</table>'+
			'</div>',
		title:'错误提示'
