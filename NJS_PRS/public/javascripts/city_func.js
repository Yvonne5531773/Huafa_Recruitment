...
var residency = {
	...
	Chk : function(id){
		if(flag==1){
			$('#origin').val(ja[id]).css("color", "#333");
			$('#residency').val(id);
		}else if(flag==2){
            $('#basicInfo #domicile').val(ja[id]).css("color", "#333");
        }else if(flag==3){
            $('#family #domicile').val(ja[id]).css("color", "#333");
        }else{
			$('#btn_hukou').val(ja[id]);
			$('#hukou').val(id);
		}
...
function residencySelect(){
	...
	var dragHtml ='<div id="residencyAlpha">';		//居住地
		...
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择籍贯</b><span onclick="boxAlpha()">关闭</span>');
	$('#drag_con').html(dragHtml);
	residency.Show();
	boxAlpha();
	draglayer();
}
function domicileSelect(){
    flag=2;
    var dragHtml ='<div id="residencyAlpha">';		//居住地
    dragHtml+='		<div id="maincity"></div>';	//主要城市
    dragHtml+='		<div id="allProv"></div>';	//所有省市
    dragHtml+='</div>';
    $('#drag_h').html('<b>请选择户籍地</b><span onclick="boxAlpha()">关闭</span>');
    $('#drag_con').html(dragHtml);
    residency.Show();
    boxAlpha();
    draglayer();
}
function fdomicileSelect(){
    flag=3;
    var dragHtml ='<div id="residencyAlpha">';		//居住地
    dragHtml+='		<div id="maincity"></div>';	//主要城市
    dragHtml+='		<div id="allProv"></div>';	//所有省市
