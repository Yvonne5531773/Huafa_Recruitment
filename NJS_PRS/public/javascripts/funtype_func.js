...
/* ****************************** 单选 ********************************* */
	...
	Show2 : function(flag){
		...
		for (var i in fun_a){
			if(i.substring(2)=='00'){
				output+='<li onclick="Funtype.SubLayer2(\''+i+'\', \''+flag+'\')">'+fun_a[i]+'</li>';
			}
		}
		$('#drag').width('670px');
		$('#FuntypeList').html('<ul>'+output+'</ul>');
	...
	SubLayer2 : function(id, flag){
		...
		for (var i in fun_a){
			if(i.substr(0,2)==myid){
				if(i.substr(2)=='00'){
					output+='<h4 onclick="Funtype.Chk2(\''+id+'\', \''+flag+'\')"><a href="javascript:">'+fun_a[id]+'</a></h4>';
				}else{
					output+='<li><a href="javascript:" onclick="Funtype.Chk2(\''+i+'\', \''+flag+'\')">'+fun_a[i]+'</a></li>';
					len++;
				}
			}
		}
	...
	Chk2 : function(id, flag){
		$('#btn_FuntypeID_2').val(fun_a[id]);
        if(flag == 1) $('#post').val(fun_a[id]).css("color", "#333");
        if(flag == 2) $('#position').val(fun_a[id]).css("color", "#333");
		$('#FuntypeID_2').val(id);
		$("#sublist").empty().hide();
		boxAlpha();
	}
...
// 单选
function funtypeSelect_2(flag){
	var dragHtml ='<div id="FuntypeAlpha">';			//职能类别
		dragHtml+='		<div id="FuntypeList"></div>';	//职能类别列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择职务类别</b><span onclick="boxAlpha()">关闭</span>');
	$('#drag_con').html(dragHtml);
	Funtype.Show2(flag);
	boxAlpha();
	draglayer();
