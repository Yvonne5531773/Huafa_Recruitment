...
var certificateService = require('../services/certificate.app.service');
...
function upsertCertificate(req, res, next){
    ...
    async.waterfall(
        [
            function(callback) {
                certificateService.upsertCertificate(certificate, function(err, result) {
                    if (err) {
                        callback(null, {});
                    } else {
            ...
            function(obj, callback) {
                if(!_.isEmpty(obj)) {
                    ...
                    }else if(_.isUndefined(resume.certificate)||_.isEmpty(resume.certificate)) resume.certificate = [];
                    if(-1 === _.indexOf(resume.certificate,obj._id)) resume.certificate.push(obj._id);
            ...
            function(obj, callback) {
                if(!_.isEmpty(obj)) {
                    resumeService.upsertResume(obj, function (err, result) {
                        ...
                        else callback(null, {resume: result, certificate: obj.certificate});
                    });
                }
            }
        ],
        function(err, result){
            ...
            resume.certificate = result.certificate;
...
function deleteCertificate(req, res, next){
    var _id = req.body.id,
        resume = req.session.applicant.resume;
    async.waterfall(
        [
            function(callback){
                certificateService.deleteCertificate(_id, function(err, result){
                    if (err) {
                        callback(err, null);
                    }else{
                        callback(null, true);
            ...
            function(obj, callback){
                if(!obj) callback(null, null);
                    ...
                    _.remove(resume.certificate, function(obj) { return obj === _id });
